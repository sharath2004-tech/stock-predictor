...
// app.js
const API_URL = "http://127.0.0.1:8000"; // Update to production backend if needed
...
const Utils = {
    ...
    setLoading(isLoading) {
        ...
        if (analyzeBtn) {
            analyzeBtn.disabled = isLoading;
            analyzeBtn.innerHTML = isLoading ? '🔄 Analyzing...' : '🔮 Analyze & Predict';
        }
    },
    validateInput(ticker, days) {
        if (!ticker) throw new Error("Please enter a stock ticker symbol!");
        if (days < 1 || days > 365) throw new Error("Please enter a valid number of days (1-365)!");
        return true;
    },
    formatPercentage(value) {
        return `${value >= 0 ? '+' : ''}${value.toFixed(2)}%`;
...
// --- Firebase Auth ---
async function signup() { /* ...same as your original code... */ }
async function login() { /* ...same as your original code... */ }
async function loginAsDemo() { /* ...same as your original code... */ }
async function logout() { /* ...same as your original code... */ }
...
async function loadCandlestick() {
    ...
    try {
        ...
        // Fetch stock and prediction in parallel
        const [stockResp, predResp] = await Promise.all([
            fetch(`${API_URL}/stock-data?ticker=${ticker}`),
            fetch(`${API_URL}/predict?ticker=${ticker}&days=${days}&model=${model}`)
        ]);

        if (!stockResp.ok) {
            const err = await stockResp.json().catch(() => ({}));
            throw new Error(err.detail || `Stock fetch failed (${stockResp.status})`);
        }
        if (!predResp.ok) {
            const err = await predResp.json().catch(() => ({}));
            throw new Error(err.detail || `Prediction failed (${predResp.status})`);
        }

        const stockData = await stockResp.json();
        const predictionData = await predResp.json();

        AppState.lastAnalysis = { ticker, model, days, stockData, predictionData, timestamp: new Date() };
        ...
        console.error("Analysis error:", error);
        Utils.showMessage("error-message", `Analysis failed: ${error.message}`, "error");
    } finally {
        Utils.setLoading(false);
    }
}
...
// --- Display Performance ---
function displayModelPerformance(predictionData, ticker, model) {
    const el = document.getElementById("performance-info");
    if (!el) return;

    const perf = predictionData.model_performance;
    const preds = predictionData.predictions;
    let content = `<h3>📊 Analysis Results for ${ticker}</h3>`;
    if (perf) {
        content += `
            <div class="metrics-grid">
                <span class="metric">Model: ${perf.model_used}</span>
                <span class="metric">Accuracy: ${(perf.r2_score*100).toFixed(1)}%</span>
                <span class="metric">MSE: ${perf.mse}</span>
                <span class="metric">Prediction days: ${preds.length}</span>
            </div>`;
        let interpretation = '';
        if (perf.r2_score >= 0.7) interpretation = '🎯 High accuracy';
        else if (perf.r2_score >= 0.5) interpretation = '⚠️ Moderate accuracy';
        else interpretation = '🚨 Low accuracy';
        content += `<p style="margin-top:10px;font-weight:600;">${interpretation}</p>`;
    }
    if (preds && preds.length > 0) {
        const first = preds[0], last = preds[preds.length-1];
        const change = last.PredictedPrice - first.PredictedPrice;
        const changePct = (change / first.PredictedPrice)*100;
        content += `<div style="margin-top:10px;padding:10px;background:rgba(255,255,255,0.7);border-radius:8px;">
            <strong>Prediction Summary:</strong><br>
            Expected change: ${Utils.formatCurrency(change)} (${Utils.formatPercentage(changePct)})<br>
            Target price: ${Utils.formatCurrency(last.PredictedPrice)}
        </div>`;
    }
...
function createStockChart(stockData, predictionData, ticker, model) {
    if (typeof ChartUtils === 'undefined') {
        Utils.showMessage("error-message","ChartUtils not loaded","error");
        return;
    }
    ...
    const predictionTrace = ChartUtils.createPredictionTrace(predictions,model,ChartUtils.getModelColor(model));
    const traces = [candlestickTrace,predictionTrace];
    if (stockData.data.length>=20) traces.push(ChartUtils.createMovingAverageTrace(stockData,20,"#ffa726"));
    if (stockData.data.length>=50) traces.push(ChartUtils.createMovingAverageTrace(stockData,50,"#ef5350"));

    const layout = ChartUtils.createLayout(ticker);
    layout.annotations = [{xref:'paper',yref:'paper',x:0.02,y:0.98,xanchor:'left',yanchor:'top',text:`Generated on ${new Date().toLocaleDateString()}`,font:{size:10,color:'#718096'},showarrow:false}];
    const config = ChartUtils.createConfig();

    Plotly.newPlot("chart", traces, layout, config).then(()=>{
        AppState.currentChart={data:traces,layout,config};
        const chartContainer = document.getElementById("chart-container");
        if(chartContainer) chartContainer.classList.add('slide-up');
    }).catch(err=>{
        console.error('Chart error:',err);
        Utils.showMessage("error-message","Failed to create chart","error");
    });
}

// --- Export ---
function exportAnalysis() {
    if (!AppState.lastAnalysis) return Utils.showMessage("error-message","No analysis to export","error");
    const data = {...AppState.lastAnalysis,exportedAt:new Date().toISOString()};
    const blob = new Blob([JSON.stringify(data,null,2)],{type:'application/json'});
    const a = document.createElement('a');
...
document.addEventListener('DOMContentLoaded',()=>{ 
    const analyzeBtn = document.querySelector('.analyze-btn');
    if(analyzeBtn) analyzeBtn.addEventListener('click',loadCandlestick);
});
